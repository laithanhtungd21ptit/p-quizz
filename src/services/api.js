...
export const createQuiz = async (quizData) => {
  try {
    ...
    formData.append(
      "quiz",
      new Blob([JSON.stringify(quizRequest)], { type: "application/json" })
    );
    ...
    for (let i = 0; i < quizData.questions.length; i++) {
      ...
      if (q.imageUrl && q.imageUrl.startsWith("data:")) {
        try {
          ...
          const blob = await base64Response.blob();
          const file = new File([blob], `question-${i + 1}.png`, {
            type: "image/png",
          });
    ...
    console.log("FormData created:", {
      ...
      questionImagesDetails: questionImages.map((f) => ({
        name: f.name,
        size: f.size,
        type: f.type,
      })),
...
export const addFavorite = async (quizId) => {
  try {
    const response = await api.post("/api/favorites/add", null, {
      params: { quizId },
    });
    return response.data;
  } catch (error) {
...
export const addQuestions = async (questionRequests) => {
  try {
    ...
    formData.append(
      "requests",
      new Blob([JSON.stringify(requestsJson)], { type: "application/json" })
    );
    ...
    for (let i = 0; i < questionRequests.length; i++) {
      ...
      if (img && typeof img === "string" && img.startsWith("data:")) {
        ...
        const empty = new File([new Blob([])], `question-${i + 1}-empty`, {
...
export const updateQuestion = async (questionId, questionData) => {
  try {
    ...
    if (questionData && questionData.request) {
      ...
      if (
        typeof questionData.imageUrl === "string" &&
        questionData.imageUrl.startsWith("data:")
      ) {
    ...
    if (questionData.imageFile instanceof File) {
      ...
      const blob = await base64Response.blob();
      const file = new File([blob], `question-${questionId}.png`, {
        type: blob.type || "image/png",
      });
      formData.append("image", file);
    }
...
export const getNextQuestion = async (pinCode, clientSessionId) => {
  try {
    ...
    console.log("=== GET NEXT QUESTION API RESPONSE ===");
...
export const savePlayerHistory = async (pinCode, clientSessionId) => {
  try {
    ...
    const response = await fetch(
      `http://localhost:8080/api/player-answers/save/${pinCode}/${clientSessionId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
